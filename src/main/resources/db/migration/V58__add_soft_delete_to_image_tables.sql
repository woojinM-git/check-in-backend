...
-- 2. roomImage 테이블에 복합 인덱스 추가 (조회 성능 최적화)
-- (roomIdx, contentId, status, imageOrder) 복합 인덱스로 WHERE와 ORDER BY 모두 커버
CREATE INDEX idx_roomImage_room_status_order ON roomImage(roomIdx, contentId, status, imageOrder);

-- 3. hotelImage 테이블에 status, deletedAt 컬럼 추가
ALTER TABLE hotelImage 
ADD COLUMN status INT NOT NULL DEFAULT 1 COMMENT '0: 삭제됨, 1: 활성',
ADD COLUMN deletedAt TIMESTAMP NULL COMMENT '삭제 시간';

-- 4. hotelImage 테이블에 복합 인덱스 추가 (조회 성능 최적화)
